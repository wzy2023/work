...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.userSession.findMany.useQuery>[0],
    option?: Parameters<typeof api.userSession.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.userSession.create.useMutation>[0],
  update?: Parameters<typeof api.userSession.update.useMutation>[0],
  remove?: Parameters<typeof api.userSession.update.useMutation>[0],
}

export const useUserSessionCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.userSession.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.userSession.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.userSession.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.userSession.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
