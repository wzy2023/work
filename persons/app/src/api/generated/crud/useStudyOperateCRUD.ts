...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.studyOperate.findMany.useQuery>[0],
    option?: Parameters<typeof api.studyOperate.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.studyOperate.create.useMutation>[0],
  update?: Parameters<typeof api.studyOperate.update.useMutation>[0],
  remove?: Parameters<typeof api.studyOperate.update.useMutation>[0],
}

export const useStudyOperateCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.studyOperate.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.studyOperate.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.studyOperate.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.studyOperate.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
