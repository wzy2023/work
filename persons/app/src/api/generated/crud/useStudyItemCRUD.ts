...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.studyItem.findMany.useQuery>[0],
    option?: Parameters<typeof api.studyItem.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.studyItem.create.useMutation>[0],
  update?: Parameters<typeof api.studyItem.update.useMutation>[0],
  remove?: Parameters<typeof api.studyItem.update.useMutation>[0],
}

export const useStudyItemCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.studyItem.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.studyItem.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.studyItem.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.studyItem.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
