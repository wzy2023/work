...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.aiRole.findMany.useQuery>[0],
    option?: Parameters<typeof api.aiRole.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.aiRole.create.useMutation>[0],
  update?: Parameters<typeof api.aiRole.update.useMutation>[0],
  remove?: Parameters<typeof api.aiRole.update.useMutation>[0],
}

export const useAiRoleCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.aiRole.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.aiRole.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.aiRole.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.aiRole.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
