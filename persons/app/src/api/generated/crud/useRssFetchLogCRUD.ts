...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.rssFetchLog.findMany.useQuery>[0],
    option?: Parameters<typeof api.rssFetchLog.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.rssFetchLog.create.useMutation>[0],
  update?: Parameters<typeof api.rssFetchLog.update.useMutation>[0],
  remove?: Parameters<typeof api.rssFetchLog.update.useMutation>[0],
}

export const useRssFetchLogCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.rssFetchLog.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.rssFetchLog.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.rssFetchLog.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.rssFetchLog.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
