...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.taskEdge.findMany.useQuery>[0],
    option?: Parameters<typeof api.taskEdge.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.taskEdge.create.useMutation>[0],
  update?: Parameters<typeof api.taskEdge.update.useMutation>[0],
  remove?: Parameters<typeof api.taskEdge.update.useMutation>[0],
}

export const useTaskEdgeCRUD = (option: Option = {}) => {
  ...
  const baseCreateState = api.taskEdge.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.taskEdge.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.taskEdge.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
