...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.rssItem.findMany.useQuery>[0],
    option?: Parameters<typeof api.rssItem.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.rssItem.create.useMutation>[0],
  update?: Parameters<typeof api.rssItem.update.useMutation>[0],
  remove?: Parameters<typeof api.rssItem.update.useMutation>[0],
}

export const useRssItemCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.rssItem.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.rssItem.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.rssItem.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.rssItem.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
