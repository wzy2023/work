...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.orderDemand.findMany.useQuery>[0],
    option?: Parameters<typeof api.orderDemand.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.orderDemand.create.useMutation>[0],
  update?: Parameters<typeof api.orderDemand.update.useMutation>[0],
  remove?: Parameters<typeof api.orderDemand.update.useMutation>[0],
}

export const useOrderDemandCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.orderDemand.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.orderDemand.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.orderDemand.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.orderDemand.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
