...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.prompt.findMany.useQuery>[0],
    option?: Parameters<typeof api.prompt.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.prompt.create.useMutation>[0],
  update?: Parameters<typeof api.prompt.update.useMutation>[0],
  remove?: Parameters<typeof api.prompt.update.useMutation>[0],
}

export const usePromptCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.prompt.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.prompt.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.prompt.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.prompt.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
