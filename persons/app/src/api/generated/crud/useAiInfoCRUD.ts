...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.aiInfo.findMany.useQuery>[0],
    option?: Parameters<typeof api.aiInfo.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.aiInfo.create.useMutation>[0],
  update?: Parameters<typeof api.aiInfo.update.useMutation>[0],
  remove?: Parameters<typeof api.aiInfo.update.useMutation>[0],
}

export const useAiInfoCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.aiInfo.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.aiInfo.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.aiInfo.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.aiInfo.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
