...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.buttonItem.findMany.useQuery>[0],
    option?: Parameters<typeof api.buttonItem.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.buttonItem.create.useMutation>[0],
  update?: Parameters<typeof api.buttonItem.update.useMutation>[0],
  remove?: Parameters<typeof api.buttonItem.update.useMutation>[0],
}

export const useButtonItemCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.buttonItem.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.buttonItem.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.buttonItem.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.buttonItem.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
