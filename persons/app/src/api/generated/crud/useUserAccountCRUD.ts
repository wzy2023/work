...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.userAccount.findMany.useQuery>[0],
    option?: Parameters<typeof api.userAccount.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.userAccount.create.useMutation>[0],
  update?: Parameters<typeof api.userAccount.update.useMutation>[0],
  remove?: Parameters<typeof api.userAccount.update.useMutation>[0],
}

export const useUserAccountCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.userAccount.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.userAccount.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.userAccount.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.userAccount.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
