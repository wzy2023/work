...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.rssFeed.findMany.useQuery>[0],
    option?: Parameters<typeof api.rssFeed.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.rssFeed.create.useMutation>[0],
  update?: Parameters<typeof api.rssFeed.update.useMutation>[0],
  remove?: Parameters<typeof api.rssFeed.update.useMutation>[0],
}

export const useRssFeedCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.rssFeed.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.rssFeed.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.rssFeed.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.rssFeed.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
