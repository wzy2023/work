...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.userItem.findMany.useQuery>[0],
    option?: Parameters<typeof api.userItem.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.userItem.create.useMutation>[0],
  update?: Parameters<typeof api.userItem.update.useMutation>[0],
  remove?: Parameters<typeof api.userItem.update.useMutation>[0],
}

export const useUserItemCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.userItem.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.userItem.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.userItem.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.userItem.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
