...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.collecting.findMany.useQuery>[0],
    option?: Parameters<typeof api.collecting.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.collecting.create.useMutation>[0],
  update?: Parameters<typeof api.collecting.update.useMutation>[0],
  remove?: Parameters<typeof api.collecting.update.useMutation>[0],
}

export const useCollectingCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.collecting.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.collecting.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.collecting.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.collecting.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
