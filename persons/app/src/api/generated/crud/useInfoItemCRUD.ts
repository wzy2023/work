...
interface Option {
  showTip?: boolean,
  list?: false | {
    query?: Parameters<typeof api.infoItem.findMany.useQuery>[0],
    option?: Parameters<typeof api.infoItem.findMany.useQuery>[1],
  },
  create?: Parameters<typeof api.infoItem.create.useMutation>[0],
  update?: Parameters<typeof api.infoItem.update.useMutation>[0],
  remove?: Parameters<typeof api.infoItem.update.useMutation>[0],
}

export const useInfoItemCRUD = (option: Option = {}) => {
  const { list, create, update, remove, showTip = true } = option

  const apiUtils = api.useUtils()

  const listState = api.infoItem.findMany.useQuery(list ? list.query : {}, {
  ...
  const baseCreateState = api.infoItem.create.useMutation({
    ...create,
    onSuccess: (...args) => {
      if (create?.onSuccess) {
  ...
  const baseUpdateState = api.infoItem.update.useMutation({
    ...update,
    onSuccess: (...args) => {
      if (update?.onSuccess) {
  ...
  const baseRemoveState = api.infoItem.update.useMutation({
    ...remove,
    onSuccess: (...args) => {
      if (remove?.onSuccess) {
